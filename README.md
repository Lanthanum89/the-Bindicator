...
The hero card's progress indicator is an SVG: a gently curved vertical path running from "today" down to the collection-day dot, with a small mascot icon (matching whichever bin is due) travelling along it via `path.getPointAtLength()` — the same technique used for the route map in [Shrimpy Swims](https://lanthanum89.github.io/Swim-Progression-App/), adapted from a horizontal km-based route to a vertical week-based countdown. It resets to empty at Friday midnight (the day after collection) and reaches full by the end of Thursday (collection day). The path's stroke colour doubles as the accent colour (matching the dot/mascot elsewhere), so it communicates both "how close" and "which bins." A small live clock (current date/time, just for reference) sits beside it. The full calendar table only shows today and future dates; past collections drop off automatically.
